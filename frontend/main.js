...
const pick = (v, fb) => (v && v.startsWith('_') ? fb : v);

  const PRODUCT_API_BASE_URL  = pick('_PRODUCT_API_URL_',  'http://localhost:8000');
  const ORDER_API_BASE_URL    = pick('_ORDER_API_URL_',    'http://localhost:8001');
  const CUSTOMER_API_BASE_URL = pick('_CUSTOMER_API_URL_', 'http://localhost:8002');  // <-- NEW
